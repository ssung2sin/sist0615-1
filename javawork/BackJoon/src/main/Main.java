...
public class Main {
	...
	public static void main(String[] args) {
		...
		if(n==1) {
			for(int i=0;i<dice.length;i++) {
				tot.add(BigInteger.valueOf(dice[i]));
			}
			tot.subtract(BigInteger.valueOf(max1));
			result=tot;
			System.out.println(result);
			return;
		}
		else if(n>=2) {
			tot.add(BigInteger.valueOf(min3*4));
			totNum-=4*3;
			tot.add(BigInteger.valueOf(min2*(8*(n-1)-4)));
			totNum-=(8*(n-1)-4)*2;
			tot.add(BigInteger.valueOf(min1*totNum));
			result=tot;
			System.out.println(result);
			return;
		}
